...
const LoginScreen = ({ route, navigation }) => {
  ...
  console.log(type)
  AsyncStorage.getItem('@type').then(data => {
    if (type == '') {
      setType(data)
    }
  })
    ...
    <View style={styles.container}>
      ...
      <ScrollView>
        <View style={[styles.row, styles.toploginRegistrationButtonContainer]}>
          <View style={[styles.backgroundBlue, styles.flex, styles.toploginRegistrationButton]} uppercase={false} ><Text minimumFontScale={2} style={[styles.textWhite, styles.toploginRegistrationText]}>{t('Prisijungimas')}</Text></View>
          <View style={[styles.background, styles.flex, styles.toploginRegistrationButton]} uppercase={false}><Text onPress={() => { navigation.navigate("RegistrationScreen") }} style={[styles.textCM, styles.toploginRegistrationText]}>{t('Registracija')}</Text></View>
        </View>
        <View><Text style={[styles.textWhite, styles.LoginText, { paddingTop: 25 }]}>{t('Prie_Scovo')}</Text></View>
