...
const NotificationsW = () => {
    ...
    console.log('pranesimai', skelbimai)
                        ...
                        <View style={{ marginTop: 5, alignItems: 'center' }}>
                            <View style={square} >
                                ...
                                <View style={[border, { backgroundColor: 'white', flex: 1, padding: 10 }]}>
                                    {item.split(',')[0] == 'matchas' ? (
                                        <View>
                                            <Text style={mainText} >{t('SutapoteSu')}</Text>
                                            <Text style={imonesPavadinimas}>{item.split(',')[1]}</Text>
                                            <Text style={imonesPavadinimas}>{item.split(',')[2]}</Text>
                                        </View>
                                    ) : (
                                        <View>
                                            <Text style={mainText}>{item.split(',')[1]} {t('turi_darbo_Skelbima')}</Text>
                                        </View>
                                    )
                                    }
                                    <View style={[susi]}>

                                        <View style={[border, { height: 30 }]} >
                                            <TouchableOpacity onPress={() => {
                                                setEl(item.split(',')[2])
                                                setModalVisible(true)
                                            }} style={{ height: 20 }}>
                                                <View style={[border, under]} ></View>
                                                <Text style={[border, { paddingLeft: 5, paddingRight: 5, height: '100%', fontSize: 12, color: '#121212', backgroundColor: 'white' }]}>{t('Susisiekti')}</Text>
...
const modalLooks = { width: 250, height: 120, backgroundColor: "white", position: 'relative', borderRadius: 8 }
